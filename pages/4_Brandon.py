...
st.title('brandon Sentiment Analysis')
...
st.markdown('Let us first look at the evolution of the sentiment of the tweets about brandon through the show:')
...
st.markdown('### Let us now look at the most frequent words used in the tweets about brandon:')
...
dates_2 = st.multiselect(
    'Which dates do you want to look at?',
    dates)
# Wordcloud with positive tweets
for date in dates_2:
    temp_df = pd.read_csv(f'./data_web_app/{date}.csv')
    # get the tweets where sentiment is the option and candidate is in it
    positive_tweets = temp_df['text'][(temp_df['sentiment'] == option) & (temp_df['text'].str.lower().str.contains('brandon'))]
